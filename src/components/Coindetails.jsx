...
const Coindetails = () => {
  ...
  const dbtn = ["0.5m", "24h", "7d", "14d", "30d", "60d", "200d", "1y", "max"];
  ...
  if (error) {
    return <Errorcom msg={"Error while Feaching data From Server"} />;
  }
        ...
        <>
          ...
          <VStack spacing={"2"} padding={"8"} alignItems={"flex-start"}>
            ...
            <Stat>
              ...
              <StatNumber>
                {sign}
                {exchange.market_data.current_price[curr]}
              </StatNumber>
              <StatHelpText>
                <StatArrow
                  type={
                    exchange.market_data.price_change_percentage_24h > 0
                      ? "increase"
                      : "decrease"
                  }
                />
            ...
            <CustomBar
              high={`${sign}${exchange.market_data.high_24h[curr]}`}
              low={`${sign}${exchange.market_data.low_24h[curr]}`}
            />
            <Box w={"full"} p={"4"}>
              <Item
                title={"Market Cap"}
                value={`${sign}${exchange.market_data.market_cap[curr]}`}
              />
              <Item
                title={"Circulating Supply"}
                value={`${exchange.market_data.circulating_supply}`}
              />
              <Item
                title={"Max Supply"}
                value={`${exchange.market_data.max_supply}`}
              />
              <Item
                title={"All Time Low"}
                value={`${sign}${exchange.market_data.atl[curr]}`}
              />
              <Item
                title={"All TIme HIgh"}
                value={`${sign}${exchange.market_data.ath[curr]}`}
              />
            </Box>
          </VStack>
        </>
